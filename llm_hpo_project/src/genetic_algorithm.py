...
def run_proxy_training(hparams, device="mps", base_model=None, max_eval_samples=4, max_train_steps=150, cached=None):
    """
    ...
    # Limitar físicamente el tamaño de lote para MPS y usar grad accumulation para el batch lógico
    max_phys_batch = 8 if device == "mps" else batch_size
    phys_batch = max(1, min(batch_size, max_phys_batch))
    accum_steps = max(1, math.ceil(batch_size / phys_batch))
    effective_seq_len = max(64, min(int(hparams["seq_len"]), 768 if device == "mps" else hparams["seq_len"]))
...
def evaluate_individual(individual, device="mps", base_model=None, max_eval_samples=4, cached=None, max_train_steps=150):
    """
