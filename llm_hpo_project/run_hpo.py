...
def parse_args():
    ...
    parser.add_argument("--train-steps", type=int, default=30, help="Pasos de fine-tune por individuo")
...
def run_baseline(args):
    ...
    metrics = evaluate_model(model, tokenizer, val_data, device=args.device or "mps", max_samples=args.eval_samples)
...
def main():
    ...
    if not args.skip_dataset:
        ...
        build_dataset(pdf_path=args.pdf_path, output_dir=args.output_dir, force_rebuild=args.force_rebuild)
        print("\n[MAIN] Dataset ready!")
    else:
